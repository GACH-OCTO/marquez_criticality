...
# generates dataset
def dataset(name, schema=None, dataSource=None, ns=namespace):
    facets = {}
    if schema is not None:
        facets.update({"schema": schema})
    if dataSource is not None:
        facets.update({"dataSource": dataSource})
...
# create dataset data
for i in range(0, 5):
    user_counts = dataset("tmp_demo.user_counts",None,datasource_dataset.DatasourceDatasetFacet("GCP", "https://storage.googleapis.com/nom-du-bucket/objet"),)
    user_history = dataset(
        ...
        datasource_dataset.DatasourceDatasetFacet("AWS", "https://nom-du-seau.s3.region.amazonaws.com"),
        "snowflake://",
    )
